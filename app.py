...
<style>
    ...
    .stTextInput input { border-radius: 6px; border: 1px solid #ddd; padding: 10px 12px; font-size: 0.95rem; }
    .stButton button { border-radius: 6px; border: none; background-color: #111; color: white; padding: 8px 20px; font-size: 0.9rem; }
    .stButton button:hover { background-color: #333; }
    .answer-box { margin-top: 2rem; padding-top: 1.5rem; border-top: 1px solid #eee; line-height: 1.6; font-size: 0.95rem; }
</style>
...
if ask and question:
    with st.spinner(""):
        try:
            answer = st.session_state.loop.run_until_complete(ask_cortex(question))
            st.markdown(f"<div class='answer-box'>{answer}</div>", unsafe_allow_html=True)
        except Exception as e:
            st.error(f"Something went wrong: {type(e).__name__}")
